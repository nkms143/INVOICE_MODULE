...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function navigateTo(pageId) {
        ...
        // Kill all page active classes
        document.querySelectorAll('.page').forEach(p => {
            p.classList.remove('active');
            // Force hide just in case class toggle isn't enough
    ...
    function _invoiceItemRowAdder() {
        ...
        tr.innerHTML = `
            ...
            <td><select class="inv-item-desc" style="width:100%;box-sizing:border-box;"></select></td>
        ...
        try {
            ...
            if (fyFilter && fyFilter.options.length <= 1) {
                ...
                if (sumRes.ok) {
                    ...
                    summaries.forEach(s => {
                        if (![...fyFilter.options].some(opt => opt.value === s.financial_year)) {
                            const opt = document.createElement('option');
                            opt.value = opt.textContent = s.financial_year;
                            fyFilter.appendChild(opt);
                        }
                    });
                }
                $(fyFilter).off('change').on('change', () => loadPaymentsList());
            }

            const fy = fyFilter ? fyFilter.value : '';
            const res = await fetch(`${API_BASE}/payments/report${fy ? '?fy=' + fy : ''}`);
    ...
    window.exportPaymentsToExcel = function () {
        const fy = document.getElementById('payments-fy-filter')?.value || '';
        window.open(`${API_BASE}/payments/export${fy ? '?fy=' + fy : ''}`, '_blank');
    };
        ...
        try {
            ...
            items.forEach((item, idx) => {
                ...
                tr.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-link" onclick="editItemMaster('${item.id}')" style="color:var(--primary);"><i class="fas fa-edit"></i></button>
    ...
    window.openItemModal = function (itemId = null) {
        ...
        if (itemId) {
            ...
            fetch(`${API_BASE}/items/${itemId}`).then(res => res.json()).then(it => {
                document.getElementById('item-id').value = it.id;
                document.getElementById('item-description').value = it.description;
                document.getElementById('item-hsn').value = it.hsn_sac || '';
                document.getElementById('item-unit').value = it.default_unit || 'NOS';
                document.getElementById('item-gst').value = it.gst_rate;
            });
        } else {
    ...
    window.editItemMaster = function (id) {
        openItemModal(id);
    };

    window.submitItemMaster = async function (e) {
        e.preventDefault();
        const id = document.getElementById('item-id').value;
        const payload = {
            description: document.getElementById('item-description').value,
            hsn_sac: document.getElementById('item-hsn').value,
            default_unit: document.getElementById('item-unit').value || 'NOS',
            gst_rate: parseFloat(document.getElementById('item-gst').value)
        };
        ...
        try {
            ...
            if (res.ok) {
                showToast(id ? 'Item updated.' : 'Item added to catalog.');
                closeItemModal();
                loadItemsList();
            }
    ...
    async function setupItemAutocomplete(rowElement) {
        const descSelect = rowElement.querySelector('.inv-item-desc');
        ...
        try {
            ...
            $(descSelect).select2({
                ...
                data: options,
                width: '100%',
                dropdownParent: $(rowElement).closest('#invoice-new-page')
            });

            $(descSelect).on('select2:select', function (e) {
                const selectedData = e.params.data;
                const matched = selectedData.data || items.find(it => it.description === selectedData.id);
                ...
                if (matched) {
                    ...
                    recalcRow(rowElement.querySelector('.inv-item-qty') || descSelect);
                }
            });

            // Handle type-in (new tags)
            $(descSelect).on('select2:clear', function () {
                recalcRow(rowElement.querySelector('.inv-item-qty') || descSelect);
            });
